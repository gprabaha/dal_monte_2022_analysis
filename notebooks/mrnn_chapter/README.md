...
| 01b | `01b_partner_identity.ipynb` | The ladder's single-vs-pair comparison confounds "the network helps" with "a bigger network helps" — a pair has more parameters than a single region. This holds architecture fixed and asks whether a region needs *this* partner or any partner: every region's units split into two independent, non-overlapping halves, so a self-pair (a region with an independent half of itself) has exactly the parameter count of a cross-pair. 4×4 matrix of (scored region, partner) cells from 10 unique two-block networks. | 10 × 5 = **50**, in progress |
