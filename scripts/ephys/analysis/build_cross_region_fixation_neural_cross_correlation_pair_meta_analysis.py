"""Build cross-region date-level pair meta-analysis for fixation neural xcorr outputs."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build cross-region date-level pair meta-analysis for fixation neural xcorr outputs.",
    )
    ...
    for signal_column in signal_columns:
        ...
        print(
            "[analysis] cross-region neural xcorr pair meta-analysis ["
    ...
    if not args.no_show_example:
        for signal_column in signal_columns:
            ...
            if not paths:
                print(f"\n[example] No cross-region pair meta-analysis output files found for {signal_column}.")
                continue
            print(f"\n[example] cross-region pair meta-analysis signal={signal_column}")
            print_fixation_neural_cross_correlation_pair_meta_analysis_example(
                paths[0],
