...
def main() -> None:
    ...
    if not args.no_show_example:
        for signal_column in signal_columns:
            ...
            if not paths:
                print(f"\n[example] No within-region output files found to preview for {signal_column}.")
                continue
            print(f"\n[example] within-region signal={signal_column}")
            print_fixation_neural_cross_correlation_example(
                paths[0],
