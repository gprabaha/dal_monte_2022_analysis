"""Plot fixation population PCA cumulative explained-variance summaries."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Plot cumulative explained-variance curves for fixation population PCA "
    ...
    out = plot_fixation_population_pca_explained_variance_cumulative(
        settings,
        regions=args.region,
        output_filename=str(
            variant_cfg.get(
                "plot_cumulative_variance_output_filename",
                cfg.get(
                    "population_pca_plot_cumulative_variance_output_filename",
                    "population_pca_explained_variance_cumulative",
                ),
            )
        ),
    )
    if not out:
        print("[plot] no cumulative explained-variance figure generated")
        return
    print(f"[plot] cumulative explained-variance figure: {out.get('output_path')}")
