...
- `fix_crosscorr_leader_follower.py`
  Derives per-session leader/follower calls from within-session cross-correlation traces, adds leader-vs-follower fixation count deltas, and summarizes session-level, date-level, pair-level, and independent global counts including consistency of `leader_fixation_count - follower_fixation_count`.
