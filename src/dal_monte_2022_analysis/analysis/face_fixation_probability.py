"""Compute face fixation probability stats within sessions and across sessions."""
...
def _build_interactive_rows(
    ...
    for key in tqdm(shared_keys, desc="Interactive-period face fixation", unit="session"):
        date, session = key
        m1_vec, m1_name = _load_face_vector(m1_paths[key], settings.face_label)
        m2_vec, m2_name = _load_face_vector(m2_paths[key], settings.face_label)
        if m1_vec is None or m2_vec is None:
...
def run_interactive_face_fixation_probability_analysis(
    settings: FaceFixationProbabilitySettings,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Run interactive-period face fixation probability analysis and persist outputs."""
    cfg = load_dataset_config(settings.cfg_path)
    m1_paths, m2_paths = _index_agent_paths(cfg, settings.input_modality)
    try:
    ...
    if not interactive_paths:
        print(
            ...
            "face fixation probability outputs."
        )
