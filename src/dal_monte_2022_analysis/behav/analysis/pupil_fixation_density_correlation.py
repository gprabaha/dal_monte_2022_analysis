...
def _pearson_correlation_with_alignment(
    pupil_vec: np.ndarray,
    density_vec: np.ndarray,
) -> tuple[float, int, int]:
    """Return Pearson r with min-length alignment and finite-value filtering."""
    ...
    p_valid = p[valid]
    d_valid = d[valid]
    p_centered = p_valid - np.mean(p_valid)
    d_centered = d_valid - np.mean(d_valid)

    denom = float(np.sqrt(np.sum(p_centered**2) * np.sum(d_centered**2)))
    if denom <= 0.0:
        return float("nan"), aligned_len, n_valid

    r = float(np.sum(p_centered * d_centered) / denom)
...
def _build_one_session_rows(args) -> list[dict]:
    ...
    for pupil_agent, (pupil_vec, pupil_monkey_name) in pupil_vectors.items():
        for density_source, (density_vec, density_monkey_name) in density_vectors.items():
            pearson_r, aligned_len, n_valid = _pearson_correlation_with_alignment(
                pupil_vec,
                density_vec,
            )
            rows.append({
