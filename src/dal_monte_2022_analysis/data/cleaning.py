"""Cleaning utilities for pruning and interpolating gaze-related data."""
...
def prune_timeline(timeline: NeuralTimelineData):
    """Drop NaNs from a neural timeline and return the valid indices."""
    t = np.asarray(timeline.t)
    valid_idx = np.where(~np.isnan(t))[0]
    if valid_idx.size == 0:
        return None, None
    pruned = NeuralTimelineData(
...
def interpolate_nans(array, *, kind="linear", window_size=10, max_nans=3):
    """Fill NaNs in 1D or 2D arrays using linear or sliding-window interpolation."""
...
def interpolate_position(
    ...
    *,
    ...
    """Interpolate 2D position samples after pruning the timeline."""
...
def prune_and_interpolate_session(
    ...
    *,
    ...
    """Prune a session timeline and interpolate position/pupil for shared agents."""
    ...
    for agent in agents:
        # Only clean agents that have both position and pupil data.
        ...
        cleaned_pupils[agent] = interpolate_pupil(
            pupils_by_agent[agent],
            valid_idx,
        )
