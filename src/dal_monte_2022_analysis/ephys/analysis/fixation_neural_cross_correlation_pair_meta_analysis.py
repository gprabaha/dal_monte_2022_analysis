"""Aggregate fixation neural xcorr outputs into per-pair date-level summaries."""
...
class FixationNeuralCrossCorrelationPairMetaAnalysisSettings:
    """Configuration for date-level neural xcorr pair meta-analysis."""
...
def build_fixation_neural_cross_correlation_pair_meta_analysis_settings_from_config(
    *,
    ...
    """Build pair meta-analysis settings from dataset + task config paths."""
    if (
    ...
    *,
    ...
    for session_row in rows:
        ...
        for xrow in xcorr_df.itertuples(index=False):
            ...
            if lag_axis is not None:
                if lag_axis.size != trace.size:
                    raise ValueError(
                        ...
                        f"date={date} pair meta-analysis for {signal_column}."
                    )
            else:
            ...
            session_token = _as_optional_str(getattr(xrow, "session", None)) or str(session_row["session"])
            bucket = grouped.setdefault(key, {"traces": [], "sessions": []})
            bucket["traces"].append(trace)
            bucket["sessions"].append(session_token)
            used_row_count += 1
    ...
    for key, bucket in grouped.items():
        ...
        valid_sessions = [
            bucket["sessions"][idx]
            for idx, keep in enumerate(valid_fixation_mask.tolist())
            if keep
        ]
        sessions_joined = _join_tokens(valid_sessions)
        n_sessions = 0 if sessions_joined is None else len(sessions_joined.split("|"))
        ...
        summary_rows.append(
            {
                ...
                "sessions": sessions_joined,
                "n_fixations": int(stacked.shape[0]),
                "mean_cross_correlation": mean_trace.astype(np.float32),
    ...
    *,
    ...
    for signal_column in signal_columns:
        ...
        if show_progress and iterator:
            iterator = tqdm(
                iterator,
                desc=f"{analysis_kind} pair meta {signal_column}",
                unit="date",
            )
        ...
        for date, date_rows in iterator:
            ...
            csv_df = csv_df.drop(columns=["mean_cross_correlation"], errors="ignore")
...
def run_within_region_fixation_neural_cross_correlation_pair_meta_analysis(
    ...
    *,
    ...
    """Aggregate within-region fixation neural xcorr outputs by date and pair."""
...
def run_cross_region_fixation_neural_cross_correlation_pair_meta_analysis(
    ...
    *,
    ...
    """Aggregate cross-region fixation neural xcorr outputs by date and pair."""
    return _run_fixation_neural_cross_correlation_pair_meta_analysis(
        settings,
...
def iter_fixation_neural_cross_correlation_pair_meta_analysis_output_paths(
    *,
    ...
    """List date-level pair meta-analysis output files for one analysis kind."""
...
def print_fixation_neural_cross_correlation_pair_meta_analysis_example(
    ...
    *,
    ...
    """Print a compact preview from one saved pair meta-analysis output."""
    ...
    if df.empty:
        print(f"[example] Pair meta-analysis output exists but is empty: {path}")
        return

    row = df.iloc[0]
    mean_trace = np.asarray(row.get("mean_cross_correlation"), dtype=float).reshape(-1)
    preview = mean_trace[: max(1, int(max_lags))]
    print("\nExample fixation neural xcorr pair meta-analysis output:")
    print(f"  file: {path}")
    ...
    print(f"  mean_cross_correlation[:{len(preview)}]: {np.array2string(preview, precision=4)}")
