...
- `fix_crosscorr_leader_follower.py`
  Loads leader-follower monkey-role pupil, fixation-count, or fixation-duration summary/session tables and renders per-monkey leader-vs-follower violin panels; also supports a pooled pupil leader-vs-follower violin with per-monkey paired mean overlays using raw per-session pupil arrays.
